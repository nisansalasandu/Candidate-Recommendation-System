...
if __name__ == '__main__':
    ...
    app.run(debug=True, host='0.0.0.0', port=5000) - Flask Backend API
...
# Global storage for CVs and Jobs (in production, use a database)
cvs_data = []
jobs_data = []
...
def upload_cv():
    ...
    try:
        ...
        required_fields = ['candidate_id', 'name', 'skills', 'experience', 'education']
        ...
        cv_entry['cleaned_text'] = clean_text(combined_text)
        ...
        return jsonify({
            ...
            'candidate_id': cv_entry['candidate_id'],
...
def upload_job():
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['job_id', 'title', 'required_skills', 'experience_required']
        for field in required_fields:
            if field not in data:
                return jsonify({'error': f'Missing required field: {field}'}), 400
        ...
        job_entry['cleaned_text'] = clean_text(combined_text)
...
@app.route('/api/recommend', methods=['POST'])
def recommend():
    """Compute recommendations for a specific job or all jobs"""
    try:
        data = request.json
        job_id = data.get('job_id', None)  # If None, compute for all jobs
        top_n = data.get('top_n', 5)  # Number of top candidates to return
        
        if len(cvs_data) == 0:
            return jsonify({'error': 'No CVs uploaded yet'}), 400
            
        if len(jobs_data) == 0:
            return jsonify({'error': 'No job descriptions uploaded yet'}), 400
        ...
        # Fit on CVs and transform
        cv_vectors = vectorizer.fit_transform(cvs_df['cleaned_text'])
        
        # Transform job descriptions
        job_vectors = vectorizer.transform(jobs_df['cleaned_text'])
        ...
        for job_idx, job in jobs_df.iterrows():
            ...
            actual_idx = job_idx if job_id else job_idx
            
            # Get similarity scores for this job
            scores = similarity_matrix[jobs_df.index.get_loc(job_idx)]
            
            # Get indices of top N candidates
            top_indices = np.argsort(scores)[::-1][:top_n]
            
            for rank, idx in enumerate(top_indices, start=1):
                candidate = cvs_df.iloc[idx]
        ...
        # Sort by job_id and rank
        recommendations = sorted(recommendations, key=lambda x: (x['job_id'], x['rank']))
